...
class NokogiriTest < MiniTest::Test
  ...
  def test_get_all_tires_belonging_to_bobsbikes_using_xpath
    ...
    results = doc.css('xmlns|tire', xmlns: "http://bobsbikes.com/")
  ...
  def test_list_all_hrefs_on_page
    ...
    results = doc.css('a').map {|ele| ele.attributes['href']}
  ...
  def test_array_of_strings_of_every_path_on_page
    ...
    results = doc.css('a').map {|ele| ele.attributes['href'].value}
  ...
  def test_get_path_for_first_link_that_has_sibling_element_of_time_class
    ...
    result = doc.css('time').each do |ele|
      
      ele.parent.children.each do |el| 
        if el.name != "time"
          return el['href']
        end
      end  
  ...
  def test_list_all_paths_of_links_in_archive_portion_of_page
    ...
    doc.css('a').each do |node| 
      if node.parent.name == "p"
        results << node.attribute("href").value 
      end  
    end
  ...
  def test_finds_first_comment
    ...
    result = doc.css('*').find do |ele|
      if ele.attributes['class'] && ele.attributes['class'].value == "comment"
        return ele
      end
    end
  ...
  def test_finds_first_links_in_comments
    ...
    results = doc.css('a').reduce([]) do |arr, ele|
      if ele.attributes['rel'] && ele.attributes['rel'].value.include?("nofollow")
        arr << ele.attributes['href'].value
      end
      arr
    end
